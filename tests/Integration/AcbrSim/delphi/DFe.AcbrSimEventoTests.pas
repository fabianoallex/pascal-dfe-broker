...
type
  ...
  public
    ...
    [Test] procedure ChaveInexistente_RegistraNaoVinculado136;
    [Test] procedure AutorDivergenteDoDestinatario_ViraManifestacaoRejeitada575;
    [Test] procedure CienciaAposConfirmacao_ViraManifestacaoRejeitada655;
    ...
    [Test] procedure JustificativaAcentuada_PreservaOsAcentos_AssinaturaEXsdValidos;
    [Test] procedure DhEvento_SaiComHoraDeBrasiliaEFusoMenos03;
    [Test] procedure FimAFim_CienciaAutomatica_PublicaDocumentoEEventoDeCiencia;
    [Test] procedure VerificacaoDeAmbiente_DestaMaquina_Manifestacao_EstaCompleta;
    [Test] procedure PastaMinimaDeXsds_BastaParaEnviarEvento;
    [Test] procedure SemXsdsDeEvento_EnviarEvento_LevantaAmbienteIndisponivel;
  end;
...
begin
  ...
  Assert.IsTrue(Pos('<xJust>', FTransmissor.UltimoEnvelope) = 0, 'desconhecimento nao leva xJust');
  ...
  Assert.IsTrue(Pos('<xJust>', FTransmissor.UltimoEnvelope) > 0, 'operacao nao realizada leva xJust');
...
begin
  LEv := Manifestador(NovoClient).EnviarEvento(Certificado, Comando(DFE_EVENTO_MANIFESTACAO_CIENCIA));
  Assert.AreEqual(DFE_EVENTO_MANIFESTACAO_CIENCIA, LEv.TipoEvento);
  Assert.IsTrue(Pos('<cStat>136</cStat>', LEv.XmlPayload) > 0, 'payload traz o cStat 136');
  Assert.IsTrue(Pos('<procEventoNFe', LEv.XmlPayload) > 0, 'payload e o procEventoNFe');
  Assert.AreEqual(1, FSim.EventosRegistrados);
  AssertSemViolacoes;
end;
...
begin
  ...
  LEv := Manifestador(NovoClient).EnviarEvento(Certificado, Comando(DFE_EVENTO_MANIFESTACAO_CIENCIA));
  Assert.AreEqual(DFE_EVENTO_MANIFESTACAO_REJEITADA, LEv.TipoEvento);
  Assert.IsTrue(Pos('<cStat>575</cStat>', LEv.XmlPayload) > 0);
...
begin
  ...
  LEv := LManif.EnviarEvento(Certificado, Comando(DFE_EVENTO_MANIFESTACAO_CIENCIA));
  Assert.AreEqual(DFE_EVENTO_MANIFESTACAO_REJEITADA, LEv.TipoEvento);
  Assert.IsTrue(Pos('<cStat>655</cStat>', LEv.XmlPayload) > 0);
  Assert.AreEqual(1, FSim.EventosRegistrados);
end;
...
begin
  ...
  Assert.AreEqual(LJust, TextoDoAcbr(ExtrairTag(FTransmissor.UltimoEnvelope, 'xJust')), 'xJust com o acento (chegou: ' + HexDe(ExtrairTag(FTransmissor.UltimoEnvelope, 'xJust')) + ')');
  LOk := AssinaturaValida(FTransmissor.UltimoEnvelope, LMsg);
  Assert.IsTrue(LOk, 'assinatura confere com o acento no digest (' + LMsg + ')');
  AssertSemViolacoes;
end;
...
begin
  ...
  Assert.AreEqual(DFE_FUSO_BRASILIA, LSufixo, 'dhEvento com sufixo -03:00 (chegou: ' + LDh + ')');
  ...
  Assert.IsTrue(Abs(LEscrito - LEsperado) < 2 / 1440, 'hora escrita (' + LDh + ') e a de Brasilia diferem mais de 2 min');
  AssertSemViolacoes;
end;
...
begin
  ...
  Assert.IsTrue(AmbienteCompleto(R), 'ambiente da manifestacao deveria estar completo:' + sLineBreak + FormatarRelatorio(R));
end;
...
begin
  ...
  try
    LManif.EnviarEvento(Certificado, Comando(DFE_EVENTO_MANIFESTACAO_CIENCIA));
    Assert.Fail('esperava EDFeAmbienteIndisponivel');
  except
    on E: EDFeAmbienteIndisponivel do
      Assert.IsTrue(Pos('faltam: envEvento_v1.00.xsd', E.Message) > 0, 'diz o que falta: ' + E.Message);
  end;
  Assert.AreEqual(0, FTransmissor.Requisicoes);
end;
