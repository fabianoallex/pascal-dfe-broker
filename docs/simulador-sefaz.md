...
2. **Delphi — projeto de integração criado, ainda NÃO rodado**: `tests/Integration/AcbrSim/delphi/AcbrSimDelphiTests.dproj` (DUnitX, **Win64**, no `PascalDfeBroker.groupproj`), espelho dos 31 testes do FPC gerado pelo conversor. Motivo: o Serviço Windows é Delphi-only e o ACBr trata `String` diferente por compilador (`docZip.XML` é `String`, `RetInfEvento.XML` é `AnsiString`), então **o encoding de `XmlDecodificado`/`XmlPayload` no Delphi é desconhecido até rodar**; os testes de acento (`Acentos_ChegamComoUtf8AoLote`, `FimAFim_AcentosChegamAoPayloadPublicado`) esperam Unicode correto e, se falharem, a mensagem mostra os ordinais do que chegou. Escritos sem compilar (Delphi só pela IDE): esperar 1–2 rodadas de ajuste. Diferenças do FPC: literais acentuados são caracteres (não bytes) via `{$IFDEF FPC}`; o DUnitX **não tem `Assert.Ignore`**, então sem libxml2/XSDs os 14 testes de evento **falham** com o prefixo `AMBIENTE NAO PREPARADO`. Depois de rodar: documentar em `docs/architecture.md` o contrato de encoding de `XmlPayload` para o publicador AMQP. Delphi **Win32** continuaria exigindo OpenSSL e libxml2 de 32 bits.
