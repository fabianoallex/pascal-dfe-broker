...
1. **libxml2 nativa em execução** (~~só para assinar/validar; a distribuição não usa~~ **correção posterior: é obrigatória sempre**, pois o ACBr lê toda resposta com ela — ver `docs/dependencias-runtime.md`). O ACBr carrega **um único nome**, fixo em compilação (`ACBrLibXml2Ext.LIBXML2_SO`): Windows `libxml2.dll` (ou `libxml2-2.dll` se compilar com `USE_MINGW`), Linux **`libxml2.so`**. Nesta máquina funcionou a `libxml2.dll` **x64** do **PostgreSQL 18** (`C:\Program Files\PostgreSQL8in`, já no PATH, com `libiconv-2`/`zlib1`/`icu*` ao lado). É acidental — não é dependência declarada do projeto. Existe `ACBrLibXml2Ext.LibXml2Path` (variável) para apontar a pasta da DLL.
...
1. ~~**Dependências de runtime acidentais**~~ — **feito em 2026-09-18** (parte código/documentação): `docs/dependencias-runtime.md` (o que é, como o ACBr acha, como obter, como verificar), `DFe.Ambiente` + `DFe.Ambiente.ACBr` (verificador), `tools/verificar-ambiente` (CLI), `EDFeAmbienteIndisponivel` (o client verifica antes de tocar no certificado/rede; o orquestrador registra e mantém a unidade agendada), e o client recusa `cStat = 0`. **Achados**: a libxml2 é obrigatória **sempre** (o ACBr lê toda resposta com ela, e sem ela engole o erro → `cStat = 0` silencioso); o conjunto **mínimo de XSDs da manifestação são 11 arquivos** (a 1ª lista, com 5, estava errada — o teste que a prova pegou); `libssl` e `libcrypto` podem vir de pastas/versões diferentes do PATH. **Falta**: o host real chamar `VerificarAmbienteACBr` na inicialização (os hosts não existem), e validar tudo no Linux.
2. **Delphi — projeto de integração criado e rodado (Win64), 2026-09-18**: `tests/Integration/AcbrSim/delphi/AcbrSimDelphiTests.dproj` (DUnitX, no `PascalDfeBroker.groupproj`), espelho dos 31 testes do FPC gerado pelo conversor. **Primeira execução: 29/31** — incluindo os 14 de evento (assinatura verificada, ponta a ponta) — e as 2 falhas eram os testes de acento, que revelaram um **bug de encoding real só do Delphi**: o ACBr entrega os bytes UTF-8 reinterpretados como Windows-1252 (`É` chegava como U+00C3 U+2030). Corrigido com `TextoDoAcbr` (`src/DFe.XmlTexto.pas`, aplicado no client) e documentado o contrato em `docs/architecture.md`, "Encoding do XML". **Reexecutado depois da correção: 31/31.** (A suíte pura Delphi também achou um bug meu no fallback de `TextoDoAcbr` — `TEncoding.UTF8.GetString` levanta `EEncodingError` para UTF-8 inválido em vez de devolver U+FFFD —, corrigido.) Diferenças do FPC: literais acentuados são caracteres (não bytes) via `{$IFDEF FPC}`; o DUnitX **não tem `Assert.Ignore`**, então sem libxml2/XSDs os 14 testes de evento **falham** com o prefixo `AMBIENTE NAO PREPARADO`; `Length()` de array dinâmico é `NativeInt` (`Int64` no Win64), então `AreEqual(n, Length(x))` exige `Integer(Length(x))`. Delphi **Win32** continuaria exigindo OpenSSL e libxml2 de 32 bits.
3. ~~**Linux**~~ — **feito em 2026-09-18** (`docs/linux.md`, `tools/docker/testar-linux.sh`): suíte pura 209/209 e integração 43/43 em Debian 12 x86_64. **Achou 3 problemas reais que o Windows escondia**: exceções de FPU (a libxml2 derrubava o processo com `EInvalidOp`), o provider padrão do OpenSSL 3.0.x inativo (nenhum `.pfx` seria lido) e o link `libxml2.so` (confirmado em execução). Corrigidos, exceto que o link `libxml2.so` é do sistema (o verificador o diz).
4. ~~**Códigos de status de evento**~~ — **feito em 2026-09-18**: NT 2012/002 v1.02 copiada em `docs/referencias/` e Anexo I do MOC 7.0 conferido (seção "Manifestação do destinatário" do README de lá). **Achou 3 erros de leitura no simulador/no código**: chave inexistente é **136 (registrado, não vinculado)**, não rejeição 494; faltavam **575** (autor ≠ destinatário), **594** (`nSeqEvento` ≠ 1) e **655** (ciência após manifestação final); e `xJust` é **só da Operação não Realizada** (o client o enviava em qualquer tipo e `InterpretarComando` o exigia no desconhecimento). Corrigidos, com testes. Ainda não modelados: 596 (prazo 180 dias), 650/651 (NF-e cancelada/denegada), 577/578/579 (datas).
5. ~~**Política de acentos no `xJust`**~~ — **decidido e feito em 2026-09-19: os acentos são preservados** (`Configuracoes.Geral.RetirarAcentos := False` no client). O XSD (`TMotivo`, `[!-ÿ]…[ -ÿ]…[!-ÿ]`) aceita Latin-1; o teste de integração prova, com o ACBr de verdade, que o `xJust` chega com o acento, que a **validação contra o XSD passa** e que a **assinatura confere** (o acento entra no digest). Consequências que o código agora trata: (a) o comprimento de 15–255 é em **caracteres**, não bytes (`DFe.XmlTexto.TamanhoEmCaracteres` — no FPC um acento são 2 bytes); (b) `InterpretarComando` faz `Trim` (o XSD proíbe espaço nas pontas) e **recusa o que o XSD rejeitaria**: aspas curvas, travessão, emoji, tab, quebra de linha (fora de U+0020..U+00FF); (c) o adaptador SOAP do simulador aponta as mesmas violações. **Confirmado também no Delphi Win64** (44/44, 2026-09-19): assinatura e XSD com acento passam. **Não verificado**: que a SEFAZ *real* aceite os acentos (só o XSD, que é o que ela publica).
6. ~~**Fuso do `dhEvento`**~~ — **medido, decidido e feito em 2026-09-19.** Achado: no **FPC 3.2.2 em Linux `Now` devolve UTC** (ignora `TZ` e `/etc/localtime`, testado com UTC/São Paulo/Manaus/Noronha), então o ACBr escrevia `dhEvento` com `+00:00`; no Windows usa o fuso do sistema (aqui `-04:00`, hora local coerente). O XSD aceita qualquer `±hh:00`, mas a NT 2012/002 (HP13) lista **só `-02:00`, `-03:00`, `-04:00`** — aceitar `+00:00` na SEFAZ não está verificado. Decisão: `src/DFe.Fuso.pas` calcula o instante em UTC (`LocalTimeToUniversal(Now)` / `TTimeZone.Local`, que usa o **mesmo** deslocamento que `Now`, então é correto mesmo quando o RTL erra o fuso) e o client escreve **hora de Brasília com `-03:00` fixo** (`AgoraDeBrasilia` + `TimeZoneConf` em `tzManual`). O Brasil não tem horário de verão desde 2019. O adaptador SOAP agora acusa fuso fora da lista da NT. Verificado: Windows (fuso `-04:00`) e Linux (UTC) escrevem `-03:00` e a hora certa; o teste falhava antes da correção nos dois.
