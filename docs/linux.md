...
3. Sob systemd, o console é o host de produção (decisão 9 do `CLAUDE.md`, "sem daemonização própria"). **O host existe** (`hosts/console/DFeBrokerConsole.dpr`, decisão 19) e foi validado neste contêiner: compila com o backend `nogui`, imprime o relatório de ambiente, sobe o broker embutido e sai com código 0 ao receber `SIGTERM` (`tools/docker/testar-linux.sh`). **Sob systemd** (`tools/docker/testar-systemd.sh`, contêiner com systemd como PID 1): a unit `hosts/console/systemd/dfe-broker.service` roda como usuário não-root, `SIGKILL` reinicia sozinho, `systemctl stop` sai com 0 e exit 2 não entra em loop — ver `hosts/console/systemd/LEIAME.md`. **Achado:** o FPC guarda a saída em buffer quando ela não é um terminal, então o log chegava ao journal atrasado (e se perdia numa queda); o host agora dá `Flush` a cada linha. **Não validado:** máquina real fora de contêiner, outras distros, e certificado real (não há). O log usa hora de Brasília com sufixo `-03:00` (item 5: `Now` é UTC aqui).
