...
- **Payload**: registrado → `procEventoNFe` completo (`RetInfEvento.XML`); rejeitado (do evento ou do lote) → retorno bruto da SEFAZ (`retEnvEvento`). **Mesma routing-key nos dois casos** — só a raiz do XML distingue. Ver o caveat de consumidor em `CLAUDE.md`, decisão 18 (decisão em aberto).
